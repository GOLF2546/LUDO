...
object Board extends App {
  ...
  val handleExistingPawn: (Player, Int, List[Player]) => Player = (player, diceValue, allPlayers) => {
    val pawnsThatCanMove = PlayerFunctions.getPawnsThatCanMove(player)

    if (pawnsThatCanMove.nonEmpty) {
      println("Your movable pawns are: " + pawnsThatCanMove.map(_.PawnId).mkString(", "))
      ...
      choice match {
        case Some(pawnId) if pawnsThatCanMove.exists(_.PawnId == pawnId) =>
          val otherPlayerPawns = allPlayers.filter(_.id != player.id).flatMap(_.pawns)
  ...
  val rollDiceAndWaitForInput: (Player, List[Player]) => Player = (player, allPlayers) => {
    val diceValue = dice()
  ...
  val playTurn: GameState => GameState = gameState => {
    ...
    val updatedPlayer = rollDiceAndWaitForInput(currentPlayer, gameState.players)
